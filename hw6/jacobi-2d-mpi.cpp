...
int main(int argc, char *argv[]) {
    ...
    for (iter = 0; iter < max_iters && gres / gres0 > tol; iter++) {
        ...
        if (mpi_j_index - 1 >= 0) {
            ...
            MPI_Send(left_edge_send, lN, MPI_DOUBLE, mpirank - 1, 125, MPI_COMM_WORLD);
            MPI_Recv(left_edge_recv, lN, MPI_DOUBLE, mpirank - 1, 126, MPI_COMM_WORLD, &status);
            for (i = 1; i <= lN; i++)
        ...
        if (mpi_j_index + 1 < num_process_per_edge) {
            ...
            MPI_Send(right_edge_send, lN, MPI_DOUBLE, mpirank + 1, 126, MPI_COMM_WORLD);
            MPI_Recv(right_edge_recv, lN, MPI_DOUBLE, mpirank + 1, 125, MPI_COMM_WORLD, &status);
            for (i = 1; i <= lN; i++)
