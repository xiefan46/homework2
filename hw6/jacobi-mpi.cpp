...
int main(int argc, char * argv[]){
  ...
  for (iter = 0; iter < max_iters && gres/gres0 > tol; iter++) {
    ...
    if(mpi_i_index - 1 >= 0){
        ...
        MPI_Send(&(lunew[topLeft]), lN, MPI_DOUBLE, top_process_rank, 123, MPI_COMM_WORLD);
        MPI_Recv(&(lunew[topLeft]), lN, MPI_DOUBLE, top_process_rank, 124, MPI_COMM_WORLD, &status);
    }
    //has down process
    if(mpi_i_index + 1 < num_process_per_edge){
        int down_process_rank = mpirank + num_process_per_edge
        MPI_Send(&(lunew[downLeft]), lN, MPI_DOUBLE, down_process_rank, 124, MPI_COMM_WORLD);
        MPI_Recv(&(lunew[downLeft]), lN, MPI_DOUBLE, down_process_rank, 123, MPI_COMM_WORLD, &status);
    }
    ...
    if(mpi_j_index - 1 >= 0){
        ...
        MPI_Send(&left_edge_send, lN, MPI_DOUBLE, down_process_rank, 125, MPI_COMM_WORLD);
        MPI_Recv(&left_edge_recv, lN, MPI_DOUBLE, down_process_rank, 126, MPI_COMM_WORLD, &status);
        for(i = 1; i <= lN ; i++)
    ...
    if(mpi_j_index + 1 < num_process_per_edge){
        ...
        MPI_Send(&right_edge_send, lN, MPI_DOUBLE, down_process_rank, 126, MPI_COMM_WORLD);
        MPI_Recv(&right_edge_recv, lN, MPI_DOUBLE, down_process_rank, 125, MPI_COMM_WORLD, &status);
        for(i = 1; i <= lN ; i++)
